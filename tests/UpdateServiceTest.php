...
final class UpdateServiceTest extends TestCase
{
    ...
    {
        if(!in_array('sqlite',PDO::getAvailableDrivers(),true))self::markTestSkipped();$db=$this->database();$settings=new SystemSettings($db);$settings->setGroup('updates',['channel'=>'prerelease']);$payload=[['tag_name'=>'v0.2.0','name'=>'Preview','html_url'=>'https://github.com/bastrian/Cogwork-Engine/releases/tag/v0.2.0','draft'=>false,'prerelease'=>true,'published_at'=>'2026-08-01T00:00:00Z','assets'=>[['name'=>'cogwork-engine-0.2.0.zip','browser_download_url'=>'https://github.com/bastrian/Cogwork-Engine/releases/download/v0.2.0/file.zip','size'=>42],['name'=>'evil.exe','browser_download_url'=>'https://evil.test/a']]],['tag_name'=>'v0.1.0','draft'=>false,'prerelease'=>true,'assets'=>[]]];$service=new UpdateService($db,$settings,fn()=>['status'=>200,'body'=>json_encode($payload,JSON_THROW_ON_ERROR),'etag'=>'"abc"','rate_remaining'=>'42','rate_reset'=>'123']);$result=$service->check(true);self::assertSame('0.2.0',$result['release']['version']);self::assertTrue($result['update_available']);self::assertCount(1,$result['release']['assets']);self::assertSame('42',$settings->get('system.update_rate')['remaining']);
    }
