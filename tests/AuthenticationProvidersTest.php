...
final class AuthenticationProvidersTest extends TestCase
{
    ...
    public function testFailedEmailDeliveryRemovesUnusableChallenge(): void
    { if(!in_array('sqlite',PDO::getAvailableDrivers(),true))self::markTestSkipped();$db=$this->database();$users=new UserService($db);$id=$users->create('mailfailure','Mail Failure','long-enough-password','user','en_US','failure@example.com');$users->setEmail($id,'failure@example.com',true);$mail=new MailService(['from_address'=>'noreply@example.com'],static fn()=>throw new \RuntimeException('delivery failed'));$service=new EmailCodeService($db,$mail,new AuthThrottle($db,'throttle-key'),'code-key');try{$service->send($id,'failure@example.com','192.0.2.9');self::fail('Delivery failure must be reported.');}catch(\RuntimeException$e){self::assertSame('delivery failed',$e->getMessage());}self::assertSame(0,(int)$db->query("SELECT COUNT(*) FROM auth_challenges WHERE purpose='email_code' AND used_at IS NULL")->fetchColumn()); }
