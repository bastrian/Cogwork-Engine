...
final class EmailCodeService
{
    ...
    public function send(string $userId,string $email,string $ip): void
    {
        ...
        $state=$this->throttle->check('email_code_send',$userId,$ip,3,600);if(!$state['allowed'])throw new \RuntimeException('Try again later.');$this->throttle->record('email_code_send',$userId,$ip,false);$code=(string)random_int(100000,999999);$now=Database::now();$this->db->prepare("UPDATE auth_challenges SET used_at=? WHERE user_id=? AND purpose='email_code' AND used_at IS NULL")->execute([$now,$userId]);$payload=json_encode(['hash'=>hash_hmac('sha256',$code,$this->key),'email_hash'=>hash('sha256',mb_strtolower($email))],JSON_THROW_ON_ERROR);$id=Database::id();$this->db->prepare('INSERT INTO auth_challenges (id,user_id,purpose,challenge_hash,payload,expires_at,used_at,created_at) VALUES (?,?,?,?,?,?,?,?)')->execute([$id,$userId,'email_code',hash('sha256',random_bytes(32)),$payload,gmdate('c',time()+600),null,$now]);try{$this->mail->send($email,'Your Cogwork Engine sign-in code',"Your one-time code is {$code}.\n\nIt expires in 10 minutes. Never share this code.");}catch(\Throwable$e){$this->db->prepare('DELETE FROM auth_challenges WHERE id=?')->execute([$id]);throw$e;}
    }
